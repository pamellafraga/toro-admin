  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Gerenciamento dos Sistemas</h1>
          <p className="text-sm text-muted-foreground mt-1">Login, senha e link das ferramentas do sistema (banco de dados, hospedagem, domínios, etc.)</p>
        </div>
        <Dialog open={isDialogOpen} onOpenChange={(open) => {
          ...
          <DialogTrigger asChild>
            <Button className="gap-2 shrink-0 w-full sm:w-auto" onClick={() => { setEditingId(null); setFormData({ service: "", login: "", password: "", url: "", notes: "", category: "FERRAMENTAS" }) }}>
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-b border-border bg-primary/5 hover:bg-primary/5">
                  <TableHead className="whitespace-nowrap py-3.5 px-4 text-[11px] font-bold uppercase tracking-wider text-foreground w-[14%]">Categoria</TableHead>
                  <TableHead className="whitespace-nowrap py-3.5 px-4 text-[11px] font-bold uppercase tracking-wider text-foreground w-[18%]">Serviço</TableHead>
                  <TableHead className="whitespace-nowrap py-3.5 px-4 text-[11px] font-bold uppercase tracking-wider text-foreground w-[18%]">Observações</TableHead>
                  <TableHead className="whitespace-nowrap py-3.5 px-4 text-[11px] font-bold uppercase tracking-wider text-foreground w-[22%]">Login</TableHead>
                  <TableHead className="whitespace-nowrap py-3.5 px-4 text-[11px] font-bold uppercase tracking-wider text-foreground w-[18%]">Senha</TableHead>
                  <TableHead className="whitespace-nowrap py-3.5 px-4 text-[11px] font-bold uppercase tracking-wider text-foreground w-[10%] text-center">Ações</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {filteredPasswords.length === 0 ? (
                  ...
                  filteredPasswords.map((pwd, idx) => (
                    <TableRow
                      key={pwd.id}
                      className={`border-b border-border/80 transition-colors hover:bg-primary/5 ${idx % 2 === 1 ? "bg-muted/20" : ""}`}
                    >
                      <TableCell className="py-3 px-4 align-middle whitespace-nowrap">
                        <span className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{pwd.category}</span>
                      </TableCell>
                      <TableCell className="py-3 px-4 align-middle">
                        <span className="text-sm font-medium text-foreground truncate block max-w-[200px]" title={pwd.service}>{pwd.service}</span>
                      </TableCell>
                      <TableCell className="py-3 px-4 align-middle max-w-[180px]">
                        <span className="text-sm text-muted-foreground truncate block" title={pwd.notes || ""}>{pwd.notes || "—"}</span>
                      </TableCell>
                      <TableCell className="py-3 px-4 align-middle">
                        <div className="flex items-center gap-2">
                          <span className="font-mono text-sm text-foreground truncate min-w-0" title={pwd.login}>{pwd.login}</span>
                          {pwd.url && (
                            <a href={pwd.url} target="_blank" rel="noopener noreferrer" className="shrink-0 p-1 rounded text-primary hover:bg-primary/10 transition-colors" title={pwd.url}>
                          ...
                          <button
                            ...
                            title="Copiar login"
                          >
                            {copiedId === `login-${pwd.id}` ? <Check className="h-4 w-4 text-green-600" /> : <Copy className="h-4 w-4" />}
                          </button>
                        </div>
                      </TableCell>
                      <TableCell className="py-3 px-4 align-middle">
                        <div className="flex items-center gap-2 whitespace-nowrap">
                          <span className="font-mono text-sm text-foreground max-w-[120px] truncate block">
                          ...
                          <button
                            ...
                            title="Copiar senha"
                          >
                            {copiedId === pwd.id ? <Check className="h-4 w-4 text-green-600" /> : <Copy className="h-4 w-4" />}
                          </button>
                        </div>
                      </TableCell>
                      <TableCell className="py-3 px-4 align-middle">
                        <div className="flex items-center justify-center gap-1">
                          <button
                            ...
                            className="inline-flex p-1.5 rounded text-muted-foreground hover:text-primary hover:bg-primary/10 transition-colors"
                            title="Editar acesso"
                          >
                          ...
                          <button
                            ...
                            className="inline-flex p-1.5 rounded text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors"
      ...
      {passwords.length === 0 && (
        <Card className="glass border-dashed border-2 border-border">
          <CardContent className="py-14 text-center">
            <p className="text-foreground font-medium mb-1">Nenhum acesso cadastrado ainda</p>
            <p className="text-sm text-muted-foreground">Adicione seus acessos clicando em &quot;Adicionar Acesso&quot;</p>
          </CardContent>
        </Card>
      )}
    </div>
